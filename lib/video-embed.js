...
export function VideoEmbed({ url, autoplay = true, muted = false, lowQuality = false, lazy = false, style }) {
  if (!url) return null;
  ...
  if (embed.type === 'youtube') {
    ...
    if (muted) src += 'mute=1&';
    if (lowQuality) src += 'quality=small&vq=small&';
    src += 'controls=1&rel=0&modestbranding=1';
    ...
    if (lazy) iframeProps.loading = 'lazy';
...
export function LazyVideoEmbed(props) {
  return <VideoEmbed {...props} lazy={true} />;
}

export function BandwidthSafeVideoEmbed(props) {
  return <VideoEmbed {...props} muted={true} autoplay={false} lazy={true} />;
}
