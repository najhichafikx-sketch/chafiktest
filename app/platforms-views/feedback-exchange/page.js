    ...
    <section className="section" style={{ paddingTop: 120 }}>
      <div className="container" style={{ maxWidth: 900 }}>
        ...
        <div className="section-header">
          ...
          <p className="section-subtitle">Submit your video and get scored on hook, editing, audio, storytelling, retention, and CTA from fellow creators.</p>
        </div>
        ...
        {tab === 'submit' && <SubmitVideo data={data} setData={setData} />}
        {tab === 'review' && <ReviewVideos data={data} setData={setData} />}
...
const REVIEW_CRITERIA = ['Hook', 'Editing', 'Audio', 'Storytelling', 'Retention', 'CTA'];

function SubmitVideo({ data, setData }) {
  ...
  const handleSubmit = () => {
    ...
    const campaign = { id, url, title, goal, status: 'pending', createdAt: Date.now(), reviews: 0, avgScore: 0, totalScore: 0 };
  ...
  return (
    <div className="glass-card" style={{ padding: 24 }}>
      ...
      <button className="btn btn-primary" style={{ width: '100%' }} onClick={handleSubmit}>Submit for Review</button>
    </div>
  );
}

function ReviewVideos({ data, setData }) {
  ...
  const handleSubmitReview = () => {
    setError('');
    const missing = REVIEW_CRITERIA.filter(c => !scores[c]);
    if (missing.length > 0) { setError('Score all criteria: ' + missing.join(', ')); return; }
    if (!whatWorked || !whatImprove) { setError('Fill in what worked and what to improve'); return; }

    const total = REVIEW_CRITERIA.reduce((sum, c) => sum + (scores[c] || 0), 0);
    const avg = total / REVIEW_CRITERIA.length;
      ...
      <div className="glass-card" style={{ padding: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
          {REVIEW_CRITERIA.map(c => (
            <div key={c}>
              <label className="form-label">{c} (1-10)</label>
              <input className="form-input" type="number" min={1} max={10} value={scores[c] || ''} onChange={e => setScores({ ...scores, [c]: parseInt(e.target.value) || '' })} />
            </div>
          ))}
        </div>
      ...
      {pending.map((camp, i) => (
        <div key={i} className="glass-card" style={{ padding: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Goal: {camp.goal}</div>
          </div>
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 24 }}>
        ...
        <div className="stat-card" style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--neon-green)' }}>{bestCat}</div>
        ...
        <div className="stat-card" style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '1.2rem', fontWeight: 700, color: '#f43f5e' }}>{worstCat}</div>
          ...
          return (
            <div key={c} style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginBottom: 4 }}>
                <span>{c}</span><span style={{ color: 'var(--neon-cyan)' }}>{score}/10</span>
              </div>
      ...
      <div className="glass-card" style={{ padding: 24, marginTop: 16 }}>
        ...
        <ul style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', lineHeight: 2 }}>
          {worstCat && <li>Focus on improving <strong>{worstCat}</strong> — it has the lowest score ({worstScore.toFixed(1)}/10).</li>}
          {bestCat && <li>Your <strong>{bestCat}</strong> is strong ({bestScore.toFixed(1)}/10). Maintain this quality.</li>}
